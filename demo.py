...
def pad(boxesA, w, h):
    ...
    dx = np.ones(numbox, np.int32)
    dy = np.ones(numbox, np.int32)
    edx = tmpw
    edy = tmph
...
def cvt_rects_to_squares(bboxA):
...
def detect_face(img, minsize, PNet, RNet, ONet, threshold, fastresize, factor):
    ...
    # 1 #
    #####
    print "1: PNet outputs #total_boxes = ", total_boxes.shape[0]
    # print total_boxes
    # return total_boxes, []
...
#        print "[2]:", total_boxes.shape[0]
        ...
        total_boxes = cvt_rects_to_squares(total_boxes)  # convert box to square
...
#        print "[5]:", total_boxes.shape[0]
        ...
        if total_boxes.shape[0] > 0:
            ...
            if len(pick) > 0:
                ...
                total_boxes = cvt_rects_to_squares(total_boxes)
                print "[8]:", total_boxes.shape[0]

        #####
        # 2 #
        #####
        print "2: RNet outputs #total_boxes = ", total_boxes.shape[0]
    ...
    # 3 #
    #####
    print "3: ONet outputs #total_boxes = ", total_boxes.shape[0]
...
def haveFace(img, facedetector):
    ...
    # toc()
    containFace = (True, False)[bboxes.shape[0] == 0]
    return containFace, bboxes
